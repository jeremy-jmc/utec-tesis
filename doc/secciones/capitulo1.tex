...
Code-BLEU is a metric designed to evaluate the quality of code generation by comparing the generated code against a set of reference codes. It is an adaptation of the BLEU metric, which is commonly used for evaluating machine translation tasks. Code-BLEU incorporates specific features relevant to programming languages, such as syntax and structure, to provide a more accurate assessment of code quality \cite{Ren2020CodeBLEU}. The metric is defined as a linear combination of several components:

\begin{equation}
    CodeBLEU = \alpha \cdot BLEU + \beta \cdot BLEU_{weight} + \gamma \cdot Match_{ast} + \delta \cdot Match_{df}
\end{equation}

where \(BLEU\) is the standard BLEU score, \(BLEU_{weight}\) is a weighted version of the BLEU score that accounts for the importance of different n-grams, \(Match_{ast}\) measures the structural similarity between the generated code and the reference code using abstract syntax trees (ASTs), and \(Match_{df}\) evaluates the data flow similarity between the two codes. The coefficients \(\alpha\), \(\beta\), \(\gamma\), and \(\delta\) are hyperparameters that can be tuned to balance the contributions of each component. And the BLEU score is defined as:

\begin{equation}
    BLEU = BP \cdot \exp\left(\sum_{n=1}^{N} w_n \cdot \log p_n\right)
\end{equation}
where \(BP\) is the brevity penalty, \(w_n\) is the weight for n-grams of length \(n\), and \(p_n\) is the precision of n-grams in the generated code compared to the reference code.
